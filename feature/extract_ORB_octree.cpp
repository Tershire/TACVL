...
int main(int argc, char **argv)
{
    ...
    const std::vector<cv::KeyPoint>& keypoints = keypoints_at_each_level[0];
    cv::drawKeypoints(image_out_ORBSLAM3, keypoints, image_out_ORBSLAM3, cv::Scalar(0, 255, 0),
            cv::DrawMatchesFlags::DEFAULT);

    cv::Mat image_show;
    cv::hconcat(image_out_CV, image_out_ORBSLAM3, image_show);
    cv::imshow("ORB Detection Comparison", image_show);
    int k = cv::waitKey(0);

    for (int level = 0; level < num_levels; ++level)
    {
        const std::vector<cv::KeyPoint>& keypoints = keypoints_at_each_level[level];
        cv::drawKeypoints(image_out_ORBSLAM3, keypoints, image_out_ORBSLAM3, cv::Scalar(0, 255, 0),
            cv::DrawMatchesFlags::DEFAULT);

        cv::resize(image_out_ORBSLAM3, image_out_ORBSLAM3, cv::Size(), 
            1.0 / scale_factor, 1.0 / scale_factor, cv::INTER_LINEAR);

        cv::imshow("Octree Keypoint", image_out_ORBSLAM3);
        k = cv::waitKey(0);
    }
